...
model = Doc2Vec(min_count=1, window=20, size=300, sample=1e-5, negative=20, workers=8, iter=4500)
print(str(model) + ", iter=" + str(model.iter))
model.build_vocab(docs)
model.train(docs2)
print(str(datetime.datetime.now()) + " Modelo treinado em: " + str(model.total_train_time))
...
resultado = []
for linha in docs:
    vetor = model.infer_vector(gensim.utils.simple_preprocess(' '.join(linha.words), deacc=True, min_len=3, max_len=301))
    try:
        mais_similar = model.docvecs.most_similar([vetor], topn=1)[0]
    except:
        continue
